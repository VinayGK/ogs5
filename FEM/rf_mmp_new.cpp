...
std::ios::pos_type CMediumProperties::Read(std::ifstream* mmp_file)
{
	...
	while (!new_keyword)
	{
		    ...
		    && (!(line_string.find("_DISTRIBUTION") != std::string::npos)))
		{
			...
			switch (porosity_model)
			{
				...
				case 10: // Chemical swelling model (constrained swelling, constant I)
				{
					...
					in >> m; // m
					if (m > 15)
						std::cout << "Maximal number of solid phases is now limited to be 15!!!"
						          << "\n";
					for (int i = 0; i < m + 1; i++)
						// molar volume [l/mol]
		...
		if (line_string.find("$TORTUOSITY") != std::string::npos)
		{
			...
			switch (tortuosity_tensor_type_name[0])
			{
				...
				case 'O': // orthotropic  <- Case Alphabet O
					...
					if (geo_dimension == 0)
						std::cout << "Error in CMediumProperties::Read: no geometric dimension"
						          << "\n";
					if (geo_dimension == 2)
					{
						in >> tortuosity_model_values[0];
		...
		if (line_string.find("$STORAGE") != std::string::npos)
		{
			...
			switch (storage_model)
			{
				...
				case 5: // Storativity is a function of normal effective stress defined by curve, set up for KTB.
					...
					pcs_name_vector.push_back("PRESSURE1");
					break;
				case 6: // Storativity is a function of normal effective stress defined by curve and distance from
				        // borehole, set up for KTB.
					in >> storage_model_values[0]; // curve number
					in >> storage_model_values[1]; // time collation
					in >> storage_model_values[2]; // Default storage value for material groups > 0
					in >> storage_model_values[3]; // Angular difference between Y direction and Sigma 1
		...
		if (line_string.find("$PERMEABILITY_TENSOR") != std::string::npos)
		{
			...
			switch (permeability_tensor_type_name[0])
			{
				...
				case 'F': // SB: read from file
					...
					break;
				default:
					std::cout << "Error in MMPRead: no valid permeability tensor type"
					          << "\n";
					break;
			}
			in.clear();
		...
		if (line_string.find("$PERMEABILITY_FUNCTION_STRAIN") != string::npos)
		{
			...
			switch (permeability_strain_model)
			{
				...
				case 5: // strain volume (threshold value, can be changed with plas strain)
					...
					in >> permeability_strain_model_value[3]; // curve numer for dependenc between threshold and plas
					                                          // strain
		...
		if (line_string.find("$PERMEABILITY_FUNCTION_PRESSURE") != std::string::npos)
		{
			...
			switch (permeability_pressure_model)
			{
				...
				case 5: // Function of overburden converted to effective stress and related to a curve.
					...
					pcs_name_vector.push_back("PRESSURE1");
					break;
				case 6: // Normal effective stress calculated according to fracture orientation, related over a curve :
				        // KTB site
					in >> permeability_pressure_model_values[0];
					in >> permeability_pressure_model_values[1];
					in >> permeability_pressure_model_values[2];
					...
					pcs_name_vector.push_back("PRESSURE1");
					break;
				case 7: // Normal effective stress calculated according to fracture orientation, related over a curve :
				        // KTB site, and distance to the borehole.
					in >> permeability_pressure_model_values[0];
					in >> permeability_pressure_model_values[1];
					in >> permeability_pressure_model_values[2];
					in >> permeability_pressure_model_values[3];
		...
		if (line_string.find("$PERMEABILITY_SATURATION") != std::string::npos)
		{
			...
			if (H3_Process)
				num_phases
				    = 3; // the existence of additional fluids in the .mfp file is not an indicator of a 4-phase system.
			else if (H2_Process)
				num_phases
				    = 2; // the existence of additional fluids in the .mfp file is not an indicator of a 3-phase system.
			else if (H_Process)
				num_phases = 1;
			//
			for (k = 0; k < num_phases; k++)
			{
				...
				switch (permeability_saturation_model[k])
				{
					...
					case 2: // krg = 1.0 - krl (only for gas phase)
						...
						{
							ScreenMessage(
							    "ERROR in MMPRead: Relative permeability model 2 is only valid for the gas phase.\n");
							exit(0);
						}
		...
		if (line_string.find("$CAPILLARY_PRESSURE") != std::string::npos)
		{
			...
			switch (capillary_pressure_model)
			{
				case 0: // k=f(Se)
					...
					if (capillary_pressure_values[2] < 0.0)
					{
						capillary_pressure_values[1]
						    = residual_saturation[0]; // old version uses relative permeabilty values for this
						capillary_pressure_values[2]
						    = maximum_saturation[0]; // old version uses relative permeabilty values for this
						old_format = true;
					}
					break;
				case 1: // const
					...
					//
					// This next value is NOT REQUIRED. It is here because in the old version this entry was used to
					// represent
					// a constant value of saturation in PP models. SHOULD DELETE THIS CHECK EVENTUALLY!
					in >> capillary_pressure_values[2];
					if (capillary_pressure_values[2] >= 0.0)
					{ // Then a constant saturation value has been entered. This is model #2.
						ScreenMessage(
						    "WARNING in MMPRead. Capillary pressure model 1 used for a constant saturation. THIS IS "
				...
				case 4: // van Genuchten
					...
					in >> capillary_pressure_values[1]; // Slr
					in >> capillary_pressure_values[2]; // Slmax
					in >> capillary_pressure_values[3]; // exponent (always <= 1.0) --> (typical is 0.5) i.e. n = 1 / (1
					                                    // - exponent) == 2.0
					...
					if (capillary_pressure_values[2] < 0.0)
					{
						...
						capillary_pressure_values[3]
						    = saturation_exponent[0]; // old version uses relative permeabilty values for this
		...
		if (line_string.find("$MASS_DISPERSION") != std::string::npos)
		{
			...
			switch (mass_dispersion_model)
			{
				...
				case 1: // Constant value
					...
					break;
				default:
					std::cout << "Error in CMediumProperties::Read: no valid mass dispersion model"
					          << "\n";
					break;
			}
			in.clear();
		...
		if (line_string.find("$HEAT_DISPERSION") != std::string::npos)
		{
			...
			switch (heat_dispersion_model)
			{
				...
				case 1: // Constant value
					...
					break;
				default:
					std::cout << "Error in CMediumProperties::Read: no valid heat dispersion model"
					          << "\n";
					break;
			}
			in.clear();
		...
		if (line_string.find("$INTERPHASE_FRICTION") != std::string::npos)
		{
			...
			if (PhaseHeatedByFriction.compare("SOLID") != 0 && PhaseHeatedByFriction.compare("FLUID") != 0
				...
				std::cout
				    << "Error in CMediumProperties::Read: $INTERPHASE_FRICTION must be either SOLID or FLUID or NONE";
			in.clear();
			continue;
		}
		//------------------------------------------------------------------------
	...
	if (heat_dispersion_model > -1)
	{
		*mmp_file << " $HEAT_DISPERSION"
		...
		switch (heat_dispersion_model)
		{
			case 1:
				// CMCD permeability
				*mmp_file << "  " << heat_dispersion_model << " " << heat_dispersion_longitudinal << " "
	...
	if (mass_dispersion_model > -1)
	{
		*mmp_file << " $MASS_DISPERSION"
		...
		switch (mass_dispersion_model)
		{
			case 1:
				// CMCD permeability
				*mmp_file << "  " << mass_dispersion_model << " " << mass_dispersion_longitudinal << " "
...
/**************************************************************************
   ...
   03/2012 JT
**************************************************************************/
double CMediumProperties::GetEffectiveSaturationForPerm(const double wetting_saturation, int phase)
{
...
double* CMediumProperties::MassDispersionTensorNew(int ip, int tr_phase) // SB + BG
{
	...
	vg = MBtrgVec(velocity, 3);
	//  if(index < 10) cout <<" Velocity in MassDispersionTensorNew(): "<<velocity[0]<<", "<<velocity[1]<<",
	//  "<<velocity[2]<<", "<<vg<<"\n";
	// test bubble velocity
	if (m_cp->bubble_velocity_model == 1)
	{
		if (index < 0)
			cout << " Velocity in MassDispersionTensorNew(): " << velocity[0] << ", " << velocity[1] << ", "
			     << velocity[2] << ", " << vg << "\n";
		...
		vg = MBtrgVec(velocity, 3);
		if (index == 100)
			std::cout << " Bubble velocity in MassDispersionTensorNew(): " << velocity[0] << ", " << velocity[1] << ", "
			          << velocity[2] << ", " << vg << "\n";
	}
	// end bubble velocity
	// Dl in local coordinates
	...
	if (this->lgpn > 0.0)
	{
		...
		// cout << " alpha_L = " << alpha_l << " < l_char/Pe; setting alpha_L = " << l_char/lgpn << " for element " <<
		// index << "\n";
		if ((set > 0) & (aktueller_zeitschritt == 1) & (component < 1) & (ip < 1))
			std::cout << "element " << index << " " << l_char << " " << alpha_l << " " << alpha_t << "\n";
	}
	//----------------------------------------------------------------------
...
double* CMediumProperties::DispersionTensorMCF(int ip, int PCSIndex, int CIndex, double* variables)
{
	...
	double set, vg, fac = 0.0, alpha_l, alpha_t, g[3] = {0., 0., 0.}, l_char = 0.0,
	                theta = Fem_Ele_Std->pcs->m_num->ls_theta;
	...
	if (this->lgpn > 0.0)
	{
		...
		if ((set > 0) & (aktueller_zeitschritt == 1) & (CIndex < 2) & (ip < 1))
			std::cout << "element " << index << " " << l_char << " " << alpha_l << " " << alpha_t << std::endl;
	}
	//----------------------------------------------------------------------
...
double CMediumProperties::Porosity(long number, double theta)
{
	...
	if (porosity_model == 11)
		for (por_index = 0; por_index < m_pcs->m_msh->mat_names_vector.size(); por_index++)
		...
		case 15:
			...
			for (size_t i = 0; i < pcs_vector.size(); i++)
				...
				if (isFlowProcess(pcs_vector[i]->getProcessType()))
				{
					...
					{
						cout << "Porosity: error getting porosity for model 15. porosity: " << porosity << " at node "
						     << number << "\n";
						porosity = porosity_model_values[0];
					}
				}
			...
			if (aktueller_zeitschritt > 1)
				for (size_t i = 0; i < pcs_vector.size(); i++)
				{
					    ...
					    || pcs_temp->getProcessType() == FiniteElement::LIQUID_FLOW)
					{
						...
						if (porosity < 1.e-6)
							cout << "error for porosity1 " << porosity << " node " << number << "\n";
					}
				}
			break;
#endif
		...
		case 15:
			...
			for (size_t i = 0; i < pcs_vector.size(); i++)
			{
				    ...
				    || (pcs_temp->getProcessType() == FiniteElement::MULTI_PHASE_FLOW))
				{
					...
					{
						std::cout << " error getting porosity model 15 " << porosity << " node " << number << "\n";
						porosity = porosity_model_values[0];
					}
				}
			}
                                                                                    ...
                                                                                    double* porosity_sw)
{
	...
	double porosity_n, porosity_IL, d_porosity, density_rock, fmon, porosity_max;
...
double CMediumProperties::PermeabilityFunctionStrain(long index, int nnodes, CFiniteElementStd* h_fem) // WW:02.08.2010
{
	...
	for (int i = 0; i < nnodes; i++)
		strainp_nodes[i]
		    = h_fem->dm_pcs->GetNodeValue(h_fem->dm_pcs->m_msh->ele_vector[index]->getNodeIndices()[i], idStrainP);
	...
	{
		for (int i = 0; i < nnodes; i++)
			strain_nodes[i] = h_fem->dm_pcs->GetNodeValue(h_fem->dm_pcs->m_msh->ele_vector[index]->getNodeIndices()[i],
			                                              idx_temp[j]);
	...
	switch (permeability_strain_model)
	{
		...
		case 5:
		{
			...
			if (permeability_strain_model_value[3] > MKleinsteZahl)
				threshold = GetCurveValue(permeability_strain_model_value[3], 0, vol_strain_temp, &gueltig);
			if (vol_strain_temp <= threshold)
				fac_perm_strain = 1 - permeability_strain_model_value[1] * (threshold - vol_strain_temp);
			else
				fac_perm_strain = 1 + permeability_strain_model_value[2] * (vol_strain_temp - threshold);
			fac_perm_strain
			    = MRange(permeability_strain_model_value[4], fac_perm_strain, permeability_strain_model_value[5]);
			break;
		}
		default:
			break;
			...
			for (j = 0; j < size; j++)
			{
				for (i = 0; i < nnodes; i++)
					stress_nodes[i] = h_fem->dm_pcs->GetNodeValue(
					    h_fem->dm_pcs->m_msh->ele_vector[index]->getNodeIndices()[i], stress_index[j]);
...
/**************************************************************************
   ...
   last modification:
**************************************************************************/
double CMediumProperties::CapillaryPressureFunction(const double wetting_saturation)
{
...
/**************************************************************************
   ...
   Last modified:
**************************************************************************/
double CMediumProperties::SaturationCapillaryPressureFunction(const double capillary_pressure)
{
	...
	switch (capillary_pressure_model)
	{
		...
		case 1: // Constant capillary pressure for ps models
			...
			ScreenMessage(
			    "Constant capillary pressure is not possible for a pressure model (PcPnw, PwPnw, or Richards).\n");
			exit(0);
			break;
		//
		case 2: // Constant saturation for pp models (for WX, from JT)
	...
	switch (capillary_pressure_model)
	{
		...
		case 4: //  Van Genuchten: 01.3.2007 WW  // 05.2010 JT.
			...
			sl = MRange(slr + DBL_EPSILON, sl,
			            slm - DBL_EPSILON); // (infinity also occurs at sl=slmax for van Genuchten)
			//
			// Convert alpha to entry pressure?
			if (entry_pressure_conversion)
				pb = (mfp_vector[0]->Density() * 9.81) / pb;
		...
		case 99: // The old iterative method. Should anyone need it (but it is somewhat inaccurate at low and high
		         // saturations)
			ds = 1.0e-2;
			do
			{
...
void CMediumProperties::SetDistributedELEProperties(string file_name)
{
	...
	while (!mmp_property_file.eof())
	{
		...
		{
			cout << "Error in CMediumProperties::SetDistributedELEProperties - no enough data sets"
			     << "\n";
			return;
		}
		//....................................................................
			...
			switch (mmp_property_dis_type[0])
			{
				...
				case 'G': // Geometric mean
					...
					for (i = 0; i < (long)_mesh->ele_vector.size(); i++)
					{
						...
						if (mmp_property_dis_type[0] == 'N')
						{
							...
							ihet = GetNearestHetVal2(i, _mesh, xvals, yvals, zvals, mmpvals);
							m_ele_geo->mat_vector(mat_vector_size) = mmpvals[ihet];
						}
						if (mmp_property_dis_type[0] == 'G')
						{
							mmpv = GetAverageHetVal2(i, _mesh, xvals, yvals, zvals, mmpvals);
				...
				case 'E': // Element data
					for (i = 0; i < (long)_mesh->ele_vector.size(); i++)
					{
						...
						if (element_area)
							_mesh->ele_vector[i]->SetFluxArea(mmp_property_value);
						if (line_string.empty())
						{
							cout << "Error in CMediumProperties::SetDistributedELEProperties - not enough data sets"
							     << "\n";
							return;
						}
					}
	...
	if ((mmp_property_name == "POROSITY") && (this->vol_mat_model == 2))
	{
		...
		for (vol_bio_index = 0; vol_bio_index < (int)_mesh->mat_names_vector.size(); vol_bio_index++)
	...
	for (k = 0; k < (int)_mesh->mat_names_vector.size(); k++)
	{
		...
		{
			cout << "Warning in CMediumProperties::WriteDistributedELEProperties: no MMP property data file to write to"
			     << "\n";
			return;
		}
...
void CMediumProperties::WriteTecplotDistributedProperties()
{
	...
	if ((long)_mesh->ele_vector.size() > 0)
	{
		...
		switch (m_ele->GetElementType())
		{
			...
			case MshElemType::PRISM:
				...
				break;
			default:
				std::cerr << "CMediumProperties::WriteTecplotDistributedProperties MshElemType not handled"
				          << "\n";
		}
	}
	//--------------------------------------------------------------------
	...
	for (i = 0; i < (long)_mesh->ele_vector.size(); i++)
	{
		...
		{
			case MshElemType::LINE:
				mat_file << m_ele->getNodeIndices()[0] + 1 << " " << m_ele->getNodeIndices()[1] + 1 << " "
				         << m_ele->getNodeIndices()[1] + 1 << " " << m_ele->getNodeIndices()[0] + 1 << "\n";
				element_type = "QUADRILATERAL";
				break;
			case MshElemType::QUAD:
				mat_file << m_ele->getNodeIndices()[0] + 1 << " " << m_ele->getNodeIndices()[1] + 1 << " "
				         << m_ele->getNodeIndices()[2] + 1 << " " << m_ele->getNodeIndices()[3] + 1 << "\n";
				element_type = "QUADRILATERAL";
				break;
			case MshElemType::HEXAHEDRON:
				mat_file << m_ele->getNodeIndices()[0] + 1 << " " << m_ele->getNodeIndices()[1] + 1 << " "
				         ...
				         << m_ele->getNodeIndices()[6] + 1 << " " << m_ele->getNodeIndices()[7] + 1 << "\n";
				element_type = "BRICK";
				break;
			case MshElemType::TRIANGLE:
				mat_file << m_ele->getNodeIndices()[0] + 1 << " " << m_ele->getNodeIndices()[1] + 1 << " "
			...
			case MshElemType::TETRAHEDRON:
				...
				break;
			case MshElemType::PRISM:
				mat_file << m_ele->getNodeIndices()[0] + 1 << " " << m_ele->getNodeIndices()[0] + 1 << " "
				         << m_ele->getNodeIndices()[1] + 1 << " " << m_ele->getNodeIndices()[2] + 1 << " "
	...
	if ((long)ele_value_dm.size() > 0) // Deformation process coupled
	{
		...
		for (i = 0; i < 3; i++)
		{
			...
			b[i] = c_coefficient[2 + i]
			       + c_coefficient[0] * exp(c_coefficient[1] * sig[i] / (T * PhysicalConstant::IdealGasConstant));
			//
		}
	}
	else
		for (i = 0; i < 3; i++)
		{
			sig[i] = c_coefficient[6 + i * 4] + c_coefficient[7 + i * 4] * xyz[0] + c_coefficient[8 + i * 4] * xyz[1]
...
void CMediumProperties::CalStressPermeabilityFactor3(double* kfac)
{
	...
	{
		am = a01
		     - (sig[i] / (-Kn + sig[i] / c_coefficient[3]) + c_coefficient[4] + c_coefficient[5] + c_coefficient[6]);
		ah[i] = am * am;
		if (ah[i] > am)
			ah[i] = am;
		//
...
/**************************************************************************
   ...
   last modification:
**************************************************************************/
double CMediumProperties::PorosityVolumetricFreeSwellingConstantIonicstrength(long index,
                                                                              double saturation,
                                                                              double temperature)
{
	// OK411
...
/**************************************************************************
   ...
   last modification:
**************************************************************************/
double CMediumProperties::PorosityEffectiveConstrainedSwelling(long index,
                                                               double saturation,
                                                               double temperature,
                                                               double* porosity_sw)
{
...
double CMediumProperties::PorosityVolStrain2(long index, double val0, CFiniteElementStd* assem) // TODO: VK
{
	...
	valnew = (val + (m_msp->Biot_Coefficient() * vol_strain_temp)) / (1 + vol_strain_temp);
	if (m_msp->SwellingPressureType == 5)
	{
		double PG = Fem_Ele_Std->interpolate(Fem_Ele_Std->NodalVal1); // Capillary pressure
		double Sw = Fem_Ele_Std->MediaProp->SaturationCapillaryPressureFunction(-PG);
...
double CMediumProperties::NonlinearFlowFunction(long index, int gp, double /*theta*/, CFiniteElementStd* assem)
{
	...
	if (flowlinearity_model==3 || flowlinearity_model==6) // Forchheimer for DLR
	{
	   ...
	   if (v_mag != 0.0) {
	       ...
	       dens_arg[1] = (1.0-theta)*assem->interpolate(assem->NodalVal_t0) +
	...
	*/
	...
	/*OK411
	   ...
	   alpha = flowlinearity_model_values[0];
	   apperture = porosity / flowlinearity_model_values[1]; //Change equivalent porosity to individual fracture
	   porosity
...
// Funktion der effektiven Spannung und des Drucks in Elementmitte
			...
			sigma = storage_model_values[2]
			        + storage_model_values[3] * InterpolValueVector(ElGetElementType(index), z, 0., 0., 0.);
...
/* Funktion der effektiven Spannung und des Drucks in Elementmitte
           ueber Kurve */
			...
			/* Spannung = sigma(z0) + d_sigma/d_z*z */
			sigma = storage_model_values[1]
			        + storage_model_values[2] * InterpolValueVector(ElGetElementType(index), z, 0., 0., 0.);
		...
		case 11:
			if (m_pcs->getProcessType() == FiniteElement::LIQUID_FLOW)
				storage = porosity_model_values[0] / (gravity_constant * mfp_vector[0]->Density());
			else
				std::cout << "Wrong process type for STORAGE model 11 (only intended for LIQUID_FLOW)." << std::endl;
			break;
		default:
...
double CMediumProperties::PermeabilityPressureFunction(long index, double* gp, double theta)
{
	...
	for (i = 0; i < no_pcs_names; i++)
	{
		...
		if (mode == 0) // Gauss point values
			...
			primary_variable[i] = (1. - theta) * GetNodeVal(number, nidx0) + theta * GetNodeVal(number, nidx1);
		else if (mode == 2) // Element average value
		{
			count_nodes = ElNumberOfNodes[ElGetElementType(number) - 1];
			element_nodes = ElGetElementNodes(number);
	...
	switch (permeability_pressure_model)
	{
		...
		case 3: /* Turbulentes Fliessen */
			...
			for (i = 0; i < nn; i++)
				h[i] = GetNodeVal(element_nodes[i], p_idx1) * eins_durch_rho_g + GetNodeZ(element_nodes[i]);
			switch (ElGetElementType(index))
			{
				...
				case 2:
					...
					if (gh < MKleinsteZahl)
						...
						k_rel = max(min(pow(gh, permeability_pressure_model_values[1] - 1.0),
						                permeability_pressure_model_values[3]),
...
double CMediumProperties::PermeabilityPressureFunctionMethod2(long index, double pressure)
{
	...
	if (Type == 2 || Type == 3 || Type == 4) /*Function defined for square, triangular and cubic elements*/
	{
		...
		distance = sqrt((x_mid - x_bore) * (x_mid - x_bore) + (y_mid - y_bore) * (y_mid - y_bore)
		...
		if (distance > 2)
			R = R + (R * (distance - 2) * permeability_pressure_model_values[8]);
...
double CMediumProperties::PermeabilityPressureFunctionMethod4(long index, double pressure, double temperature)
{
	...
	thermal_stress = (permeability_pressure_model_values[2] - temperature) * permeability_pressure_model_values[3]
...
double CMediumProperties::HeatTransferCoefficient(long number, double theta, CFiniteElementStd* assem)
{
	...
	switch (heat_transfer_model)
	{
		...
		case 2:
		{
			...
			{
				std::cout << "***Error: Effective heat transfer model is not supported. "
			...
			val = 6. * (1 - n) / dp * 1.
			      / (1. / h + dp / 10.0 / lamda_s); // confirmed with DLR people. typo in Schaube2011
		}
		break;
		default:
			break;
